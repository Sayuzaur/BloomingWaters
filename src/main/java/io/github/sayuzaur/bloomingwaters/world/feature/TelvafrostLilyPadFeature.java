...
public class TelvafrostLilyPadFeature extends Feature {
    protected int flowerChance;

    public TelvafrostLilyPadFeature(int flowerChance) {
    ...
    public boolean generate(World world, Random random, int x, int y, int z) {
        ...
        if (isValidTarget(world, x, forceY, z)) {
            world.setBlockWithoutNotifyingNeighbors(x, forceY, z, BlockListener.FROST_LILY_PAD.id);
    ...
    public boolean isValidTarget(World world, int x, int y, int z) {
        if (world.isAir(x, y, z) && world.hasSkyLight(x, y, z) && BlockListener.FROST_LILY_PAD.canGrow(world, x, y, z)) {
