...
public class SinkingMudBlobFeature extends Feature {
    protected int minY;
    protected int maxY;

    public SinkingMudBlobFeature(int minY, int maxY) {
        this.minY = minY;
        this.maxY = maxY;
    }
    ...
    public boolean generate(World world, Random random, int x, int y, int z) {
        ...
        int sinkingMudId = BlockListener.SINKING_MUD.id;
        int innerSize = 1;
        ...
        //Inner part
        for (int varX = x; varX <= x + innerSize; varX++) {
            for (int varZ = z; varZ <= z + innerSize; varZ++) {
                ...
                if (canGenerate) {
                    world.setBlockWithoutNotifyingNeighbors(varX, varY, varZ, sinkingMudId);

                    if (random.nextInt(3) != 0 && canReplace(world, varX, varY - 1, varZ)) {
                        world.setBlockWithoutNotifyingNeighbors(varX, varY - 1, varZ, sinkingMudId);

                        if (random.nextInt(3) == 0 && canReplace(world, varX, varY - 2, varZ)) {
                            world.setBlockWithoutNotifyingNeighbors(varX, varY - 2, varZ, sinkingMudId);
                        }
                    }
                }
            }
        ...
        for (int varX = x - innerSize; varX <= x + outerSize; varX++) {
            for (int varZ = z - innerSize; varZ <= z + outerSize; varZ++) {
    ...
    public boolean canReplace(World world, int x, int y, int z) {
        return world.getBlockId(x, y, z) == BlockListener.MUD.id
                || world.getBlockId(x, y, z) == BlockListener.SINKING_MUD.id;
    }
